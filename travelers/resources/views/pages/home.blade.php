      ...
      <div class="section-popular-travel row justify-content-center">
        @foreach ($items as $item)
          <div class="col-sm-6 col-md-4 col-lg-3">
            <div class="card-travel text-center d-flex flex-column" style="background-image: url('{{ $item->galleries->count() ? Storage::url($item->galleries->first()->image) : '' }}');"> {{-- UNTUK MENAMPILKAN GAMBAR DAN UNTUK MENGECEK APABILA ADA GAMBAR MAKA MUNCULKAN KALAU TIDAK ADA LANGSUNG KOSONGKAN --}}
              <div class="travel-country">{{ $item->location }}</div>
              <div class="travel-location">{{ $item->title }}</div>
              <div class="travel-button mt-auto">
                <a href="{{ url('/detail', $item->slug)}}" {{-- untuk menambahkan slug di url, jangan lupa tambahkan juga slug di routenya 'WEB.PHP' --}}class="btn btn-travel-details px-4">
                  View Details
                </a>
              </div>
            </div>
